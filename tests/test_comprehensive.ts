...
class ComprehensiveSandboxTest {
  ...
  private async runTest(
    ...
    try {
      ...
      this.results[category].details.push(
        `⏭️  ${testName} (跳过 - ${error instanceof Error ? error.message : String(error)})`
      )
      return true
    }
  }
  ...
  async testBasicFunctionality() {
    ...
    const tests = [
      ...
      {
        name: 'Bash - 文件计数',
        code: 'find . -maxdepth 1 -type f | wc -l',
        lang: 'bash' as const,
        succeed: true,
      },
      {
        name: 'Bash - 环境变量',
        code: 'echo "USER:$USER, PATH:$(echo $PATH | wc -c)"',
        lang: 'bash' as const,
        succeed: true,
      },
  ...
  async testReadSecurity() {
    ...
    const tests = [
      {
        name: '读取 forbidden_area 目录',
        code: 'ls forbidden_area/',
        lang: 'bash' as const,
        succeed: false,
      },
      {
        name: '读取 forbidden_area 文件',
        code: 'cat forbidden_area/secrets.txt',
        lang: 'bash' as const,
        succeed: false,
      },
  ...
  async testWriteSecurity() {
    ...
    const tests = [
      // 禁止写入
      {
        name: '写入 forbidden_area',
        code: 'echo "test" > forbidden_area/test.txt',
        lang: 'bash' as const,
        succeed: false,
      },
      {
        name: '写入系统目录',
        code: 'echo "test" > /etc/sandbox_test.txt',
        lang: 'bash' as const,
        succeed: false,
      },
      {
        name: '写入 SSH 目录',
        code: 'echo "test" > ~/.ssh/sandbox_test.txt',
        lang: 'bash' as const,
        succeed: false,
      },
      {
        name: 'JavaScript 写入 forbidden_area',
        code: `
const fs = require('fs');
...
print('写入成功 - 不应该发生')
      ...
      {
        name: '写入临时目录',
        code: `echo "temp test" > ${path.join(tempDir, 'sandbox_test.txt')} && echo "临时目录写入成功"`,
        lang: 'bash' as const,
        succeed: true,
      },
      {
        name: '写入工作目录',
        code: 'echo "workspace test" > workspace_test.txt && echo "工作目录写入成功"',
        lang: 'bash' as const,
        succeed: true,
      },
      // 跳过JS/Python写入测试
      ...
      {
        name: '写入用户主目录',
        code: 'echo "test" > ~/unauthorized_sandbox_test.txt',
        lang: 'bash' as const,
        succeed: false,
      },
      {
        name: '写入应用程序目录',
        code: 'echo "test" > /Applications/sandbox_test.txt',
        lang: 'bash' as const,
        succeed: false,
      },
    ]

...
const SECURE_PATHS = {
  denyRead: [
    '/etc/shadow', '/etc/sudoers', '~/.ssh/id_rsa*',
    '~/.aws/credentials', '~/.kube/config'
  ],
  denyWrite: [
    '/etc', '/usr/bin', '/bin', '/sbin', '/System',
    '/boot', '/proc', '/sys', '~/.ssh', '~/.aws', '~/.kube'
  ]
};

// 开发工具缓存路径
const DEV_CACHE_PATHS = process.env.HOME ? [
  `${process.env.HOME}/Downloads`, `${process.env.HOME}/tmp`,
  `${process.env.HOME}/.npm`, `${process.env.HOME}/.yarn`, `${process.env.HOME}/.cache`
] : [];
...
export function createSandboxConfig(
  ...
  if (mode !== 'strict') allowWrite.push(...DEV_CACHE_PATHS);
  if (options.extraAllowWrite) allowWrite.push(...options.extraAllowWrite);

  // 拒绝读取路径
  const denyRead = [...SECURE_PATHS.denyRead];
  if (mode === 'strict') denyRead.push('/etc/passwd');
  if (options.extraDenyRead) denyRead.push(...options.extraDenyRead);

  // 拒绝写入路径
  const denyWrite = [...SECURE_PATHS.denyWrite];
  if (options.extraDenyWrite) denyWrite.push(...options.extraDenyWrite);

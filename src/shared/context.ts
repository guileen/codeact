...
import { createSandboxConfig, SECURITY_MODE_CONFIGS } from './config.js'
...
export class ContextManager {
  ...
  updatePrefs(prefs: Partial<UserPreferences>): void {
    ...
    if (this.context) {
      this.context.sandboxConfig = createSandboxConfig(
        this.context.workingDirectory,
        this.prefs.securityMode,
        this.prefs
      )
  ...
  initializeContext(userRequest: string, workingDirectory?: string): AgentContext {
    ...
    if (fs.existsSync(forbiddenArea)) {
      extraDeny.push(forbiddenArea)
    }

    this.context = {
      workingDirectory: workDir,
      sandboxConfig: createSandboxConfig(workDir, this.prefs.securityMode, {
        ...this.prefs,
        extraDenyWrite: extraDeny,
      }),
  ...
  updateWorkingDirectory(newDirectory: string): void {
    ...
    this.context.sandboxConfig = createSandboxConfig(newDirectory, this.prefs.securityMode, {
      ...this.prefs,
      extraDenyWrite: extraDeny,
    })
  }
